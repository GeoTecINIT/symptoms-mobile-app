...
export class AudioWavesComponent implements AfterViewInit, OnDestroy {
    ...
    constructor(private page: Page) {
        this.animationsInfo = this.keyframesNames.map((name) =>
            this.page.getKeyframeAnimationWithName(name)
        );
    }

    ngAfterViewInit(): void {
        this.animations = this.waveBars.map((waveBar) =>
            this.buildAndStartKeyframeAnimationFor(waveBar)
        );
    }
    ...
    private buildAndStartKeyframeAnimationFor(
    ...
    private buildRandomKeyframeAnimation(): KeyframeAnimation {
        const animationInfo =
            this.animationsInfo[
                Math.floor(Math.random() * this.animationsInfo.length)
            ];
        animationInfo.duration = this.randomDuration(
            this.minDuration,
            this.maxDuration
        );
